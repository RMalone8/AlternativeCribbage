...
def draw_hand(win, hand: list, discard_list: list, finished_peggging: bool) -> list:
    '''
    ...
    if not finished_peggging:
        ...
        for card in hand:
            # cards being displayed
            pos_dict = {"x": i*198 + 3, "y": 200, "width": 200, "height": 280}
            pos_info.append(pos_dict)
            card_sprite = pygame.image.load(f"Assets/Cards/Modern/{(card['suit'][0]).lower()}{card['order_value']}.png").convert_alpha()
            card_sprite = pygame.transform.scale(card_sprite, (pos_dict['width'], pos_dict['height']))
            card_rect = card_sprite.get_rect()
...
def discarding_logic(select: int, discard_list: list, crib: list, player_hand: list) -> int:
    '''
    ...
    return 0 # do not change the turn, still figuring out what to discard

def pegging_logic(selection: int, player_hands: list, player_points: list, turn: int, running_sum: int, pile: list, player_piles: list, go_count: int):
    new_points = 0
    ...
    if selection < len(player_hands[turn%2]) and player_hands[turn%2][selection]['value'] + running_sum < 32:
        ...
        turn += 1
        go_count = 0
    if selection == 6 and not check_for_playable_cards(player_hands[turn%2], running_sum) or running_sum == 31:
        go_count += 1
        # override if pile == 31
        if running_sum == 31:
            go_count = 31
        ...
        if go_count > 1:
            ...
            running_sum = 0
        turn += 1
    return go_count, turn, running_sum, new_points, #pile, player_piles, player_hands, new_points
